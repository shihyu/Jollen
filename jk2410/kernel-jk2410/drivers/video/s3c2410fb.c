 ...
 *
 * 삼성 텔레메틱스 ? : 96*320 에뮬레이션. 
 *
 *  var.xres = 96 으로 하고,
 *  fix.line_length = 240*2 로 한다.
 *  app 가 linux FB spec. 을 준수한다면 문제가 없을 것이다.
 *
 *  var.xres_virtual 을 조정하는 방법도 생각해 볼 수 있지만, spec. 과는 별도로
 *  app. 가 엄청 똑똑해야 할 뿐만 아니라, panning/wrapping 과 꼬일 염려가
 *  있으므로 배제한다.
 */
